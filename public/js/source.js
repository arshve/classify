...
class Mobilenet {
  ...
  async loadModel() {
    ...
    if (this.graphModelURL.includes("https://tfhub.dev/")) {
      this.model = await tf.loadGraphModel(this.graphModelURL, {
        fromTFHub: true
      });
    } else {
      this.model = await tf.loadGraphModel(this.graphModelURL, {
  ...
  async addImage(inputOrLabel, labelOrCallback, cb) {
    ...
    if (
      ...
      typeof inputOrLabel === "string" ||
      typeof inputOrLabel === "number"
    ) {
      imgToAdd = this.video;
      label = inputOrLabel;
    }
  ...
  async train(onProgress) {
    ...
    if (!(batchSize > 0)) {
      throw new Error(
        "Batch size is 0 or NaN. Please choose a fill the batch size."
      );
    }
  ...
  async classifyInternal(imgToPredict) {
    if (this.usageType !== "classifier") {
      throw new Error(
        "Mobilenet Feature Extraction has not been set to be a classifier."
      );
    }
    await tf.nextFrame();
    ...
    const predictedClasses = tf.tidy(() => {
      const imageResize =
        imgToPredict === this.video ? null : [IMAGE_SIZE, IMAGE_SIZE];
  ...
  async predictInternal(imgToPredict) {
    if (this.usageType !== "regressor") {
      throw new Error(
        "Mobilenet Feature Extraction has not been set to be a regressor."
      );
    }
    await tf.nextFrame();
    this.isPredicting = true;
  ...
  async load(filesOrPath = null, callback) {
    if (typeof filesOrPath !== "string") {
        ...
        if (file.name.includes(".json")) {
          ...
          fr.onload = d => {
            if (JSON.parse(d.target.result).ml5Specs) {
              this.mapStringToIndex = JSON.parse(
                d.target.result
              ).ml5Specs.mapStringToIndex;
            }
          };
          fr.readAsText(file);
      ...
      this.jointModel = await tf.loadLayersModel(
